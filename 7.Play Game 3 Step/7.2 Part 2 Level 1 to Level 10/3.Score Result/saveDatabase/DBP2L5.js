...
function Awake() { 
	textResultLevelP2L5.text = "" + PlayerPrefs.GetInt("ScoreLevelP2L5");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP2L5");						//Maximum Score
}
...
function send() {							//Send Score to Database
    ...
    form.AddField("score", textResultLevelP2L5.text);
    form.AddField("username", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP2L5.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}