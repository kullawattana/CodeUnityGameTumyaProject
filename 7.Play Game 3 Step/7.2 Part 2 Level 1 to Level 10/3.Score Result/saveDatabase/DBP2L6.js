...
function Awake() { 
	textResultLevelP2L6.text = "" + PlayerPrefs.GetInt("ScoreLevelP2L6");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP2L6");						//Maximum Score
}
...
function send() {							//Send Score to Database
    ...
    form.AddField("score", textResultLevelP2L6.text);
    form.AddField("username", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP2L6.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}