...
function Awake() { 
	textResultLevelP3L4.text = "" + PlayerPrefs.GetInt("ScoreLevelP3L4");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP3L4");						//Maximum Score
}
...
function send() {							//Send Score to Database
    ...
    form.AddField("score", textResultLevelP3L4.text);
    form.AddField("username", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP3L4.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}