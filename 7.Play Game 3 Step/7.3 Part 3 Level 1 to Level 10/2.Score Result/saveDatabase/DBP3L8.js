...
function Awake() { 
	textResultLevelP3L8.text = "" + PlayerPrefs.GetInt("ScoreLevelP3L8");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP3L8");						//Maximum Score
}
...
function send() {							//Send Score to Database
    ...
    form.AddField("score", textResultLevelP3L8.text);
    form.AddField("username", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP3L8.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}