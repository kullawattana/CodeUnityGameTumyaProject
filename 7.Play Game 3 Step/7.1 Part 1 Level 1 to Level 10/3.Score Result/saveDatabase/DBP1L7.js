...
function Awake() { 
	textResultLevelP1L7.text = "" + PlayerPrefs.GetInt("ScoreLevelP1L7");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP1L7");						//Maximum Score
}
...
function send() {							//Send Score to Database
    ...
    form.AddField("score", textResultLevelP1L7.text);
    form.AddField("username", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP1L7.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}