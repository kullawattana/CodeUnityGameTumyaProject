...
function Awake() { 
	textResultLevelP1L5.text = "" + PlayerPrefs.GetInt("ScoreLevelP1L5");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP1L5");						//Maximum Score
}

function OnTriggerEnter (other : Collider) {
	if(other.tag == "playagain")							
	{
		Application.LoadLevel("Part1 L5");				
		isPlayingAudio = true;
        audio.clip = Trigger;
        audio.Play();
	}
	...
	{
		Application.LoadLevel("Part2 L5");				
		isPlayingAudio = true;
        audio.clip = Trigger;
        audio.Play();
        send();
...
function send() {							//Send Score to Database
    ...
    form.AddField("scoreResult", textResultLevelP1L5.text);
    form.AddField("scoreMax", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP1L5.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}