...
function Awake() { 
	textResultLevelP2L8.text = "" + PlayerPrefs.GetInt("ScoreLevelP2L8");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP2L8");						//Maximum Score
}
...
function send() {							//Send Score to Database
    ...
    form.AddField("score", textResultLevelP2L8.text);
    form.AddField("username", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP2L8.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}