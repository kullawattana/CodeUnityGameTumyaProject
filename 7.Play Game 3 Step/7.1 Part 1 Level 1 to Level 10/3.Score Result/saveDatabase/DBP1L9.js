...
function Awake() { 
	textResultLevelP1L9.text = "" + PlayerPrefs.GetInt("ScoreLevelP1L9");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP1L9");						//Maximum Score
}

function OnTriggerEnter (other : Collider) {
	if(other.tag == "playagain")							
	{
		Application.LoadLevel("Part1 L9");				
		isPlayingAudio = true;
        audio.clip = Trigger;
        audio.Play();
	}
...
function send() {							//Send Score to Database
    ...
    form.AddField("score", textResultLevelP1L9.text);
    form.AddField("username", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP1L9.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}