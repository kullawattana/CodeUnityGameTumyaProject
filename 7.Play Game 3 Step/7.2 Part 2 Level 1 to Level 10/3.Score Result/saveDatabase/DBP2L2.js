...
function Awake() { 
	textResultLevelP2L2.text = "" + PlayerPrefs.GetInt("ScoreLevelP2L2");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP2L2");						//Maximum Score
}
...
function send() {										//Send Score to Database
    ...
    form.AddField("score", textResultLevelP2L2.text);
    form.AddField("username", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP2L2.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}