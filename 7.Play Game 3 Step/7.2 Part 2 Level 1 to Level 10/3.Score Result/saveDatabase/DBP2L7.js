...
function Awake() { 
	textResultLevelP2L7.text = "" + PlayerPrefs.GetInt("ScoreLevelP2L7");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP2L7");						//Maximum Score
}

function OnTriggerEnter (other : Collider) {
	if(other.tag == "playagain")							
	{
		Application.LoadLevel("Part1 L7");				
		isPlayingAudio = true;
        audio.clip = Trigger;
        audio.Play();
	}
	...
	{
		Application.LoadLevel("Part2 L7");				
		isPlayingAudio = true;
        audio.clip = Trigger;
        audio.Play();
        send();
...
function send() {							//Send Score to Database
    ...
    form.AddField("score", textResultLevelP2L7.text);
    form.AddField("username", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP2L7.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}