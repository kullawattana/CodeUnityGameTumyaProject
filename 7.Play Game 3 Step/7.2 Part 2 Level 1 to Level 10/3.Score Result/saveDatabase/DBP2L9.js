...
function Awake() { 
	textResultLevelP2L9.text = "" + PlayerPrefs.GetInt("ScoreLevelP2L9");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP2L9");						//Maximum Score
}
...
function OnTriggerEnter (other : Collider) {
	...
	{
		Application.LoadLevel("Part2 L9");				//Next Level To LogoTumya Page
		isPlayingAudio = true;
        audio.clip = Trigger;
        audio.Play();
        send();
	...
	if (Input.GetButtonDown("Fire1")) 
	{
    	...
    	if(Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), hit))
    	{
        	...
        	if(hit.collider.tag == "playagain") 
        	{
            	...
            	{
            		Application.LoadLevel("Part2 L9");	
...
function send() {							//Send Score to Database
    ...
    form.AddField("score", textResultLevelP2L9.text);
    form.AddField("username", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP2L9.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}