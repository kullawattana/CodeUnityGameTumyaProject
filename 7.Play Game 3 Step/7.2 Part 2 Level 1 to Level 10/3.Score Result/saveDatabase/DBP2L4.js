...
function Awake() { 
	textResultLevelP2L4.text = "" + PlayerPrefs.GetInt("ScoreLevelP2L4");				//My Score
	textMaxScore.text = "" + PlayerPrefs.GetInt("MaxScoreP2L4");						//Maximum Score
}
...
function send() {							//Send Score to Database
    ...
    form.AddField("scoreResult", textResultLevelP2L4.text);
    form.AddField("scoreMax", textMaxScore.text);
    var url = "http://localhost/unity_test/SaveScoreP2L4.php";
    var w = WWW(url, form);
    yield w;
    Debug.Log (w.text);
}